  ...
  <div>
    <div class="vis-component-inner">
      <div
        ...
        v-show="this.chartProp.loadingStatus"
      >
      ...
      <div
        class="corpsum-line-chart corpsum-chart"
        ref="chart"
        :key="componentKey"
      >
...
export default {
  ...
  methods: {
    drawChart() {
      ...
      select(this.$refs.chartGroup).attr(
        "transform",
    ...
    drawLines() {
      ...
      for (let i = 0; i < this.chartData[this.valueType].data.length; i += 1) {
        if (this.chartData[this.valueType].data[i] !== undefined) {

          const pathGroup = focusGroup
          .append("g")
        ...
        pathGroup
          ...
          .attr("fill", "none")
          .attr("class", () => {
            if (this.wordFormsBarIndex !== false) {
              return `line-path stroke-series-color-${this.wordFormsBarIndex} line-id-${i}`;
            }
            return `line-path stroke-series-color-${i} line-id-${i}`;
          })
        ...
        for (
          let j = 0;
          j < this.chartData[this.valueType].data[i].data.length;
          j += 1
        ) {
          pathGroup
            .append("circle")
            .attr("cx", () =>
              this.xScale(
                this.chartData[this.valueType].data[i].data[j][this.xKey]
              )
            )
            .attr("cy", () =>
              this.yScale(
                this.chartData[this.valueType].data[i].data[j][this.yKey]
              )
            )
            .attr("class", () => {
              if (this.wordFormsBarIndex !== false) {
                return `line-data-circle bg-series-color-${this.wordFormsBarIndex}`;
              }
              return `line-data-circle bg-series-color-${i}`;
            })
            .attr("r", "4")
            .style("cursor", "pointer")
            .on("mouseover", () => {
              this.onLineCircleMouseOver(
                this.chartData[this.valueType].data[i].data[j][this.xKey],
                this.chartData[this.valueType].data[i].name,
                {
                  x: this.xScale(
                    this.chartData[this.valueType].data[i].data[j][this.xKey]
                  ),
            ...
            }) // produce undefined property 'data'
            .on("mouseout", () => {
              this.onLineCircleMouseOut();
            })
            .on("click", () => {
              this.onLineCircleClick(
                this.chartData[this.valueType].data[i].data[j][this.xKey],
                this.chartData[this.valueType].data[i].name
              );
            }); // produce undefined property 'data'
    ...
    drawGridlines() {
      select(this.$refs.gridlinesY).call(
        axisLeft(this.yScale)
          .ticks(5)
          .tickSize(
    ...
    onLineCircleMouseOver(year, query, position, iParent) {
      ...
      for (let i = 0; i < this.chartData.collocations.length; i += 1) {
        ...
        if (
          d.year === "" + year.toString(10) &&
          d.query.replace(/\\/g, "") === query
        ) {
          collxData = d.data;
      ...
      if (collxData.length > 0) {
        collxGroup = select(this.$refs.tooltips)
          .append("g")
        ...
        collxGroup
          ...
          .attr(
            "class",
            () => `line-collx-rect line-path stroke-series-color-${iParent}`
          )
          .attr("x", () => position.x + 8)
          .attr("y", () => position.y - 8)
          .attr("width", 100)
    ...
    highlightLine(queryName, flag) {
      const lineID = this.getObjectKey(
        this.chartData[this.valueType].data,
        queryName,
        "name"
      ); // returns the column index on which the hovering is happening
      select(this.$refs.focusGroup)
        .select(`.line-id-${lineID}`)
        .classed("line-path-hovered", flag);
    },
    exportImage() {
      this.$refs.chart.$children[0].chart.exportChartLocal({
        type: "image/svg+xml",
      });
    },
    exportCSV() {
    ...
    showTable() {
      ...
      this.$el
        .querySelector(".highcharts-data-table")
        .childNodes[0].classList.add("table", "table-sm", "table-bordered");
      ...
      if (this.wordFormsToShow) {
        ...
        for (let i = 0; i < this.wordFormsToShow.length; i += 1) {
          ...
          key = this.getObjectKey(
            this.chartProp.wordForms.relative.data,
            `[word="${this.wordFormsToShow[i].name}"]`,
            "name"
          );
          wordFormsLines.relative.data.push(
            this.chartProp.wordForms[this.valueType].data[key]
          );
        }
